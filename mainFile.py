...
class PersonalAssistant:
    def __init__(self, recognizer, microphone, keywordlist):
        self.recognizer = recognizer
        self.microphone = microphone
        self.keywordlist = keywordlist

    def respondHelper(self, keyword):
    ...
    def listeningNow(self):
        ...
        print("Say something")
        try:
            googleGuess = self.recognizer.recognize_google(audio)
    ...
    def respond(self, keyword):
        ...
        phrase = self.respondHelper(keyword)
        tts = gTTS(phrase, 'en')
...
if __name__ == "__main__":
    ...
    cassidy = PersonalAssistant(recognizer, microphone, keywordlist)
    ...
    while not done:
        for j in range(6):  # we'll loop to keep checking for input
            ...
            guess = cassidy.recognize_speech_from_mic()
        ...
        if guess["transcription"]:  # i.e. if the value of "transcription" != None
            ...
            if audioTranscribed in keywordlist:  # if it's a word in our special keywords list
                for i in range(5):
                    try:
                        cassidy.respond(audioTranscribed)
                        break
                    except:
            ...
            elif "open" in audioTranscribed:
                audioTranscribedList = audioTranscribed.split()
                if audioTranscribedList.__len__() == 2:
                    # I want to search in the command prompt for the applicationName + ".exe" or something
                    pass
